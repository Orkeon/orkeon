...
$lines.Add('')
$lines.Add('## Campagnes exécutées')
$lines.Add('')

if ($campaigns.Count -eq 0) {
    $lines.Add("_Aucune campagne archivée pour l'instant._ Lancez-en une — commencez par Ollama, dont le coût est nul :")
    $lines.Add('')
    $lines.Add('```bash')
    ...
    $lines.Add('| Horodatage (UTC) | Provider | Modèle | ✅/❌/➖ | Version | Rapport |')
    $lines.Add('|---|---|---|---|---|---|')
    ...
    foreach ($c in $ordered) {
        $lines.Add("| $($c.Stamp) | ``$($c.Provider)`` | ``$($c.Model)`` | $($c.Status) $($c.Passed)/$($c.Failed)/$($c.Skipped) | $($c.Version) | [rapport]($($c.Relative)) |")
    }

    $lines.Add('')
    $lines.Add('## Dernière campagne par provider')
    $lines.Add('')
    $lines.Add('| Provider | Modèle | Date | Statut | Modes exercés |')
    $lines.Add('|---|---|---|---|---|')
...
$lines.Add('_Index régénéré par `lib/recap.sh` ou `lib/recap.ps1` depuis les rapports présents sur disque._')
$lines.Add("_$($campaigns.Count) campagne(s) archivée(s)._")
