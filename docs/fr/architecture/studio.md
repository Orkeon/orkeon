...
- **Presets/ & Llm/** — le catalogue de presets LLM (`LlmPresets`, `OrkeonCliDefaults`), la sonde d'endpoint (`ILlmEndpointProbe`/`HttpLlmEndpointProbe`, `LlmApiKeyResolver`), la sonde de solde (`IProviderBalanceProbe`/`HttpProviderBalanceProbe`, STUDIO-33) et les comptes que couvre une lecture de solde (`ProviderBalanceAccount`/`ProviderBalanceTarget`, STUDIO-35).
...
### Importer un cas d'usage tel quel (STUDIO-41)

En mode expert, chaque carte qui n'est pas « référence seule » propose aussi « Importer tel quel » : le cas devient une équipe de Mes équipes — prête à lancer avec ses données, et modifiable — sans passer par l'atelier. Le mode novice n'affiche jamais l'action, et un cas « référence seule » ne la propose jamais. Le nom que prend l'équipe est le titre du cas dans la langue de la fenêtre, et son dossier suit la règle des dossiers de toute équipe — l'identifiant du cas nomme le dossier quand le titre ne garde rien que cette règle puisse utiliser, comme un titre chinois. Le nom est vérifié d'abord, par la règle même de l'adoption (STUDIO-26, D-07) : quand quelque chose occupe déjà son dossier, un bandeau au-dessus des cartes dit quoi, propose le nom libre (« Importer sous « … (2) » ») et le chemin vers l'équipe qui porte ce nom, et rien n'est exporté tant que le nom n'est pas libre. Puis `UseCaseImporter` fait écrire le cas par le CLI sous la forme d'un dossier d'équipe — `orkeon usecases export <id> --to <dossier de transit> --lang <code> --events jsonl`, dans un dossier de transit nommé comme le dossier de l'équipe — et fait entrer ce dossier dans la racine des équipes par l'import que prend toute équipe (`TeamCatalog.Import` : le détecteur même du lanceur, la copie, la fiche relue et normalisée) ; le dossier de transit disparaît dans tous les cas. L'équipe porte sa crew sous la forme `crew/config.yaml`, ses données d'exemple, et une fiche qui la nomme, la décrit et enregistre les montages du manifeste relativement à l'équipe (`./data:/data:ro`, `./output:/output:rw`). « Modifier » est actif, puisque `forge reopen` reconstruit une session depuis `crew/` avec un brief dérivé du plan ; et le lanceur résout les montages sous l'équipe comme ceux de toute équipe adoptée, si bien que l'équipe se lance avec ses données montées. Le bandeau se termine sur la ligne même de l'adoption et sur le rapport de l'écran Importer sur les dossiers, avec « Ouvrir Mes équipes » ; Mes équipes se rafraîchit d'elle-même.
...
Le segment **Solde**, à droite, dit ce qu'il reste sur les comptes fournisseurs — voir *Le solde des fournisseurs*, ci-dessous.

### Le solde des fournisseurs (STUDIO-35)

Le segment **Solde**, à droite de la barre d'état, dit ce qu'il reste sur les comptes fournisseurs derrière les profils de modèle, dans les deux modes. La sonde est celle de STUDIO-33 (`IProviderBalanceProbe`, Core `Llm/`) ; côté WPF, on la lit à travers `BalanceReadings` (`ViewModels/Shell/`), que la barre, les lignes de profil et l'éditeur de profil partagent.

- **Ce qu'il couvre (D-01).** Les comptes du profil par défaut, de celui de l'assistant et des profils que nomment les équipes — une entrée par compte : un fournisseur, l'hôte de son endpoint et la variable où vit sa clé (`ProviderBalanceAccount`, Core). Deux profils sur un même compte coûtent une seule requête ; l'hôte fait partie du compte parce que les plateformes `.ai` et `.cn` de Kimi refusent chacune la clé de l'autre. Un runtime local, ou un profil sans endpoint, n'a pas de compte et n'est pas interrogé. Les équipes entrent par un seul point, `StatusBarSources.TeamProfiles`, que l'archivage (STUDIO-31) restreint aux équipes actives.
- **Quand il est lu — la politique réseau (D-02).** Au démarrage, une fois les profils chargés ; à la fin d'une activité — une exécution, un essai, une composition (la consommation en direct est l'affaire des groupes d'activité, STUDIO-34) ; et sur un clic. Rien d'autre : une fenêtre laissée ouverte n'envoie aucune requête d'elle-même, et une lecture déjà en cours est rejointe, jamais répétée. La seule exception est à activer : Réglages › Studio peut le relire toutes les 5, 15, 30 ou 60 minutes — désactivé par défaut. Un fournisseur dont la documentation établit qu'il n'expose pas de solde reçoit sa réponse sans aucune requête (STUDIO-33) : seuls les comptes DeepSeek, Kimi et OpenRouter en coûtent jamais une.
- **Ce que dit une entrée.** « DeepSeek 110,00 CNY » — les montants tels que le fournisseur les renvoie, jamais convertis ; un compte pas encore lu montre son fournisseur avec le glyphe de relecture ; une clé refusée, une absence de réponse ou une réponse hors format sont dites discrètement (« Kimi · clé refusée »). L'info-bulle donne une ligne par compte — les profils qui l'utilisent, ce qu'il lui reste ou pourquoi on ne le sait pas, l'heure de la lecture —, avec sous un échec la ligne anglaise de la sonde. Un clic relit ; sur un fournisseur dont seule la console montre le solde (non exposé, ou clé admin requise), le clic ouvre cette console (`LlmPresets.KeyConsoleFor`).
- **Le seuil (D-03).** Un seuil optionnel par fournisseur dont une clé lit le solde, dans la devise que ce fournisseur renvoie : en dessous, le montant passe en ton d'avertissement sur la barre, sur la ligne du profil et dans l'éditeur.
- **Les profils (D-04).** Chaque ligne de profil porte une pastille de solde dès que son compte a été lu dans la session. L'éditeur de profil a une ligne « Lire le solde » sous « Tester la connexion » : elle lit l'endpoint en cours d'édition avec la clé qu'une exécution présenterait — d'abord celle tapée et pas encore mémorisée —, refuse d'interroger sans clé, et propose la console là où seule la console montre le solde.
- **Rien n'est enregistré (D-05).** Une lecture vit en mémoire le temps de la session : un chiffre lu hier ne doit pas passer pour celui d'aujourd'hui, donc une nouvelle fenêtre démarre sans aucun. Aucune clé n'est jamais affichée : la sonde la masque (STUDIO-33 D-05), et les lectures sont masquées une fois de plus là où elles sont rangées.
- **Qui peut joindre un fournisseur.** Le solde est lu sans clic, donc la sonde HTTP n'est nommée qu'à un seul endroit : `MainWindowViewModel.CreateForCurrentMachine`. Une fenêtre construite sans sonde dans `StudioServices.BalanceProbe` ne lit rien et n'affiche aucun segment — tout test qui n'en nomme pas ; la campagne de captures passe la sienne, hors ligne (`OfflineBalanceProbe`), si bien que ses captures portent un solde et qu'aucune requête ne peut partir.

### Réglages › Studio (STUDIO-35)

Un septième onglet des réglages, **Studio**, dans les deux modes, réunit le comportement de Studio lui-même sur cette machine : aujourd'hui la carte du solde des fournisseurs — la lecture automatique, désactivée par défaut, et les seuils, avec à côté de chaque fournisseur le dernier montant lu, qui dit dans quelle devise saisir. Ses valeurs sont `StudioSettings` (`ViewModels/Services/`), rangées dans `ui-preferences.json` sous une section `Studio` — jamais dans le fichier de réglages des équipes — et écrites aussitôt par `StudioUiPreferences.PersistStudio`. Sa deuxième carte est la suggestion d'archivage de Mes équipes (STUDIO-32, plus bas) : son interrupteur, activé par défaut, et son seuil, soixante jours par défaut. Un nouveau réglage est une propriété de plus dans `StudioSettings`, une clé de plus dans `UiPreferencesDocument`, une carte de plus dans `StudioSettingsView`.

`ui-preferences.json` est écrit **par fusion** : `UiPreferencesDocument` (du texte au texte, vérifié sans disque) écrit les clés qu'un geste possède et laisse toutes les autres telles qu'il les a trouvées. Le thème, la langue et le mode écrits par les trois gestes de la fenêtre conservent Réglages › Studio, ses écritures les conservent, et une clé ajoutée par un Studio plus récent survit à un plus ancien.
...
de toute façon. Et l'arrivée d'une copie est son activité (STUDIO-32) : une copie fraîche d'une vieille
équipe n'est pas une vieille équipe, donc la copie oublie la dernière exécution que le sidecar de
l'original notait et inscrit `addedAt` — dans un sidecar minimal quand elle arrive sans.
...
ailleurs n'écrit rien. La **dernière activité** d'une équipe est la plus récente de quatre dates —
`lastRunAt`, sa dernière entrée d'historique des lancements (l'historique en garde cinquante), le
`promotedAt` de son `forge.json`, et l'`addedAt` d'une copie (STUDIO-32) — calculée à la lecture de
la liste, sans migration (`TeamSummary.LastActivity`, `TeamCardViewModel.LastActivity`). La dernière
exécution d'une carte est sa dernière entrée d'historique, ou le `lastRunAt` du sidecar une fois que
l'historique l'a oubliée — la date seule, l'issue étant celle de l'historique.
...
(`TargetDescription.IsArchived`), et relisent l'état d'archivage juste avant un lancement : une équipe
archivée dans leur dos — son sidecar modifié à la main, une seconde fenêtre — y est refusée aussi
(STUDIO-32) ; « Relancer » dans l'Historique lit la cible de l'entrée — pas celle
...
autour fait l'objet de la section suivante (STUDIO-32).

### Mes équipes : retrouver une équipe parmi beaucoup (STUDIO-32)

Mes équipes reste lisible avec beaucoup d'équipes. Au-dessus des cartes, trois commandes travaillent
sur les cartes déjà en mémoire — jamais par `Refresh()`, qui réinitialise la liste et fait relire par
la fenêtre les dossiers d'équipe des réglages et les références de montage :

- **la recherche** lit le nom d'une équipe et son besoin : chaque mot tapé doit commencer un de leurs
  mots, accents et casse mis à part — la règle de la galerie des cas d'usage, une seule aide pour les
  deux (`TextSearch`) ;
- **l'ordre** est la dernière activité, la plus récente d'abord — une équipe dont l'activité est
  inconnue en dernier — ou le nom. Il n'y a pas d'ordre « date de création » : aucune date fiable ne
  dit quand une équipe a été faite ;
- **« Archives (N) »**, affiché tant qu'une équipe est archivée, fait passer la liste aux équipes
  archivées (avec une ligne qui dit ce qu'une archive garde) et en revient. La vue se referme d'elle-même
  quand la dernière archive est restaurée ou supprimée.

La barre latérale compte les équipes **actives** (`TeamsViewModel.ActiveCount`) : ni la recherche ni
la vue Archives ne le font bouger. L'écran vide distingue trois états : aucune équipe (en créer ou en
importer une), toutes les équipes archivées (« Ouvrir les archives »), et une recherche qui n'a rien
trouvé (dit, avec « Effacer la recherche »).

**Archiver, puis annuler.** Une carte active propose « Archiver » — un bouton libellé en mode novice,
une icône en mode expert — et ne demande rien : l'archivage s'annule d'un clic. Un bandeau « Équipe
« … » archivée — Annuler » tient le haut de l'écran quelques secondes, sur un minuteur à lui
(`StudioServices.UndoDelay`) : abandonner une pause annule tout ce qui attend sur son minuteur, et
celui du chat porte les temps de l'assistant. Une équipe planifiée passe par « Arrêter la
planification et archiver », et une équipe occupée est refusée sur sa carte, comme STUDIO-31 l'a
posé. Annuler « Arrêter la planification et archiver » rend l'équipe, pas sa planification — le
bandeau le dit : la planification choisie revient dans le sidecar, l'inscription que le moteur a
retirée ne revient pas, et la carte affiche « Planification non installée » avec son « Installer la
planification » ; rien n'est réinstallé dans le dos de l'utilisateur. Une carte archivée propose
« Restaurer » et « Supprimer », rien d'autre — ni lancer, ni modifier, ni renommer, ni copier — sous
un badge atténué « Archivée » et la date de l'archivage. Une seule question à l'écran à la fois :
ouvrir la question d'une carte ferme le bandeau d'annulation et la question de toute autre carte.

**La suggestion d'archivage (DB-1).** Un bandeau propose « N équipes non lancées depuis 60 jours — les
archiver ? », en les nommant. Il ne fait que proposer : rien n'est archivé sans le clic, qui archive
exactement les équipes nommées, chacune relue à ce moment-là (une équipe lancée depuis est refusée sur
sa carte), et une seule annulation les rend toutes ; « Plus tard » range la proposition pour la
session. Il ne liste jamais une équipe planifiée — le système la lance sans Studio — ni une équipe
dont la dernière activité est inconnue, et il attend l'historique des lancements, qui peut connaître
une exécution plus récente que les sidecars. Il s'efface tant qu'une carte pose une question ou que
le bandeau d'annulation est affiché. Le seuil et l'interrupteur qui la coupe sont dans Réglages ›
Studio (`StudioSettings.ArchiveSuggestion`, `ArchiveSuggestionDays`, écrits dans
`ui-preferences.json` par fusion), et un changement s'applique aussitôt.
...
tant que l'équipe est la cible de l'exécution en cours sur l'écran Lancer ou Tester
...
mode, langue, thème, et Réglages › Studio (le solde de STUDIO-35, la suggestion d'archivage de STUDIO-32). À part des deux racines ci-dessus :
il n'est écrit que par l'application WPF (Windows seulement), dans les données
applicatives locales (**non itinérantes**) ; chaque écriture fusionne avec ce que le
fichier contient, et chaque accès disque est tolérant — un fichier absent, corrompu ou
non inscriptible retombe sur les valeurs par défaut, jamais sur un plantage. Une lecture
de solde n'est jamais écrite ici, ni ailleurs.
...
Studio, STUDIO-31, et l'arrivée d'une copie, STUDIO-32) —
