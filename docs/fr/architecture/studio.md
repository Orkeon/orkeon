...
- **UseCases/** — le client typé d'`orkeon usecases` (STUDIO-39) : `UseCaseClient`, jumeau de `ForgeClient` — `usecases list` exécuté jusqu'au bout pour le catalogue de la galerie, une session `usecases search` gardée ouverte pour les suggestions sous le besoin, chaque requête appariée à sa réponse par l'identifiant de corrélation, les échecs typés plutôt que levés — ainsi que les lecteurs du catalogue et d'une réponse (`UseCaseCatalog`, `UseCaseAnswer`), la règle qui décide des réponses assez proches pour être suggérées (`UseCaseSuggestions`), et la normalisation de recherche du CLI, reproduite pour compter combien de fiches partagent un terme (`UseCaseTerms`, épinglée des deux côtés par un même corpus partagé).
