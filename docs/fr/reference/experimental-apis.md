...
|---|---|---|
| `ORKEXP001` | **A2A (protocole agent-à-agent)** — `IA2AClient`, `IA2AServer`, `IA2AAgentDiscovery`, `IA2ATaskRouter`, `IAgentRegistrationStore`, les implémentations `A2A*` et `AddOrkeonA2A` | L'implémentation précède la spécification A2A v1.0 ; la passe de conformité (PUB-08) a ajouté la persistance des tâches et tranché la révocation de certificats, et l'alignement des bindings vers la v1.0 (HTTP+JSON d'abord) reste à faire — voir la [matrice de conformité](./a2a-conformance.md). |
