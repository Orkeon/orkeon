...
Démarrer ou reprendre un cycle exige un LLM configuré (`orkeon init`) : la forge refuse d'ouvrir l'entretien sans lui (`FORGE-LLM-UNAVAILABLE`) plutôt que de dégrader en silence. `list`, `promote`, `reopen`, `schedule` et `unschedule` sont entièrement hors ligne.
...
| `--name <équipe>` | *(promote)* Le nom de l'équipe : il devient le titre de `FORGE.md`, de `forge.json` et de la session elle-même — sans lui, ils gardent l'objectif du brief. Pris tel qu'écrit, tiret initial compris : la valeur de `--name` n'est jamais lue comme l'option suivante. Orkeon Studio le transmet toujours. |
