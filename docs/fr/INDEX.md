...
| Fichier | Description |
|---------|-------------|
| [Fournisseurs LLM](./architecture/llm-providers.md) | 13 providers (OpenAI, Anthropic, Azure, Groq, Ollama, etc.), adaptateurs, factory, kit de campagnes sur API réelle |
...
| [Décisions d'architecture (ADR)](./adr/README.md) | ADR-002 (shared kernel Tools.Abstractions), ADR-003 (shared kernels Analysis), ADR-004 (jumeaux de nommage scripting — remplacé par l'ADR-007), ADR-005 (famille Tools.* hétérogène), ADR-006 (sous-système RAG `src/rag/`), ADR-007 (D3 : renommage `Orkeon.Cli.Commands.Scripting`), ADR-008 (les chemins virtuels sont la seule monnaie versée aux agents) |
