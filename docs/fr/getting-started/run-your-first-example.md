...
| `401 (Unauthorized)` ou `NU1301` au restore, mentionnant une source `nuget.pkg.github.com` | Un `nuget.config` utilisateur déclare encore le flux GitHub Packages d'une ancienne installation | Inutile : les paquets `Orkeon` et le tool `orkeon` sont sur nuget.org, et le `nuget.config` du dépôt épingle nuget.org comme unique source. Retirez le flux GitHub (`dotnet nuget remove source <nom>`) — aucun token n'est requis. |
