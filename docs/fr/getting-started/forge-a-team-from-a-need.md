...
3. **L'essai.** L'équipe tourne dans un bac à sable, in-process, sur votre exemple d'entrée : les écritures sont confinées au dossier de sortie de la session, et `shell_command`/`code_interpreter` sont retirés du catalogue, tout simplement.
...
5. **Votre arbitrage.** Conforme → l'équipe est prête. Sinon : l'accepter telle quelle (vous jugez sur pièce), corriger (le diagnostic est réinjecté dans le plan, mot pour mot), ou en rester là. `--auto` arbitre seul, dans les limites du budget.
