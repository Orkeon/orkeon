...
| Capacité | Supporté |
|---|---|
| Providers vision (capacité `Vision = true`) | Les 16 : Anthropic, OpenAI, Azure OpenAI, DeepSeek (par modèle, `deepseek-v4-flash-vision-exp`), Gemini, Grok, HuggingFace, Kimi, MiniMax (par modèle, famille VL), Mistral, Ollama (`images` natif), Qwen, TogetherAI, Z.AI, OpenRouter (par modèle, non campagné) et Mammouth AI (par modèle, d'après la liste `text, image` du vendeur — non campagné) |
| Provider sans la capacité | Dégradation vers le repli texte `LlmMessage.Content`, avec un avertissement structuré nommant les parts abandonnées |
