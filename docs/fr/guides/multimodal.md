...
| Capacité | Supporté |
|---|---|
| Providers vision (capacité `Vision = true`) | Les 16 : Anthropic, OpenAI, Azure OpenAI, DeepSeek (natif sur `deepseek-flash` depuis V4.1 ; `deepseek-v4-pro` reste texte seul), Gemini, Grok, HuggingFace, Kimi, MiniMax (par modèle, famille VL), Mistral, Ollama (`images` natif), Qwen, TogetherAI, Z.AI, OpenRouter (par modèle, non campagné) et Mammouth AI (par modèle, d'après la liste `text, image` du vendeur — non campagné) |
