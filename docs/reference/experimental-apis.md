...
|---|---|---|
| `ORKEXP001` | **A2A (agent-to-agent protocol)** — `IA2AClient`, `IA2AServer`, `IA2AAgentDiscovery`, `IA2ATaskRouter`, `IAgentRegistrationStore`, the `A2A*` implementations and `AddOrkeonA2A` | The implementation predates the A2A v1.0 specification; the conformance pass (PUB-08) added task persistence and settled certificate revocation, and the binding alignment toward v1.0 (HTTP+JSON first) is still to come — see the [conformance matrix](./a2a-conformance.md). |
