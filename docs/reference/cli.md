...
orkeon forge "sort my emails by urgency" --reference 03-email-pipeline   # start from a need, modelled on a use case
...
orkeon forge rename <team-folder> --name <name>   # rename the team: folder, session, titles, schedule — all or nothing
```
...
Starting or resuming a cycle requires a configured LLM (`orkeon init`): the forge refuses to open the interview without one (`FORGE-LLM-UNAVAILABLE`) rather than degrade silently. `list`, `promote`, `reopen`, `schedule`, `unschedule` and `rename` are fully offline.
...
| `--reference <id>` | *(new session)* The use case the team is modelled on: an id of [`orkeon usecases`](#orkeon-usecases), whose crew — agents, tasks, process, each agent's tools — is shown to the assistant that designs the team as a model of **structure, not content to copy**: the brief alone decides what the team does. Shown when the team is designed, never during the interview; the tools the sandbox catalogue does not offer are removed from it, and it is bounded — each text cut at 200 characters, the whole at 4,000 (about 1,150 tokens), past which the rest of a larger team is cut, and said to be. Measured on 2026-09-24 against the real tool catalogue: a reference adds 2,100 to 4,500 characters to each call that designs the team — +590 to +1,290 estimated tokens, +800 for the median use case, on a call of about 3,000 without one. The session records it (`reference` in `session.json`: id and title) and announces it on `session.started`; a resume or a reopen composes with it again, and the option is refused anywhere but on a new session. The promotion traces it: « Inspired by: <title> (<id>) » in `FORGE.md`, next to the provenance line and in the brief's language, and `reference` in `forge.json`. An unknown id is refused with exit 1 and `USECASES-UNKNOWN-ID` before any session is created or any model called — an `error` line, then `session.finished` «failed», in `--events` mode. This is how Orkeon Studio passes the use case chosen in the wizard's gallery. |
...
| `--name <team>` | *(promote, rename)* The team's name: it titles `FORGE.md`, `forge.json` and the session itself — without it they keep the brief's goal; for `rename`, the new name, which names the folder too (below). Taken as written, a leading dash included: the value of `--name` is never read as the next option. Orkeon Studio always passes it. |
...
**Renaming a team.** `forge rename <team-folder> --name <name>` renames a promoted team, all of it or nothing. The folder takes the name's folder, next to where it is — the folder rule, `equipe` when nothing usable is left; the session rule R links to the folder follows it, as after a promotion (suffixed `-2`… past another session's name, never over it); the titles follow — `session.json` (and its `promotedTo`), `forge.json`, Orkeon Studio's `studio-team.json` (its `name` alone: every other field is kept, known to this version or not), the title line and the install command of `FORGE.md`, the launchers' header; `schedule/` is regenerated for the new path and name, and a registration the system runs is reinstalled under the new name, the former one removed. A name whose folder is the team's own only retitles. Every step is journaled first: when one fails — the disk refuses, the system refuses the new registration — everything done before it is put back, the former registration last, and the verb exits 1 saying the team was not renamed: `FORGE-RENAME-FAILED`, or the system's own `FORGE-SCHEDULE-REFUSED`. A folder already there under the new name — another team, a folder, a file — is refused with `FORGE-RENAME-TAKEN`, saying what is there; a registration that still runs a team declaring no schedule any more refuses the rename as well (`FORGE-SCHEDULE-STILL-INSTALLED`: `unschedule` it first), since it would be left running a folder that no longer exists. A copy is renamed without its original's session ever being touched, and a folder that holds no team — no `forge.json`, no `studio-team.json`, no crew — is never moved (`FORGE-TEAM-UNREADABLE`). With `--events`: the `warning`s, `session.renamed` when the session's folder moved, `schedule.state` when the schedule was reinstalled, then `team.renamed` (`from`, `path`, `name`); a refusal is one `error`, after which nothing changed. The verb takes no option but `--name` and `--events`. This is Orkeon Studio's « Rename », which also rewrites its own launch history — the one thing the CLI does not own.
