...
| EventHub middleware stages | `AddOrkeonEventHubObservability()` (logging + telemetry) · `AddOrkeonEventHubAcl(policy?)` (enforces the crews' `links:` blocks) · `AddOrkeonEventHubIdempotency(capacity?)` (point-to-point only, in-memory, does not survive the process) · `AddOrkeonEventHubValidation()` (a declared `SchemaId` must be registered — the contract's existence, not the payload's conformance). Each is independent; registration order is the pipeline order — see [EventHub §12](../architecture/event-hub-and-crew-lifecycle.md) | Infrastructure | `IEventHubMiddleware`, `ICrewLinkProvider`/`ICrewLinkRegistry`, `IEventSchemaRegistry` | Beta |
