...
| **An LLM endpoint + key** | Any of the 16 supported providers, or a local endpoint such as Docker Model Runner / Ollama. Supplied through an `appsettings` profile (below). |
...
| `401 (Unauthorized)` or `NU1301` during restore, mentioning a `nuget.pkg.github.com` source | A user-level `nuget.config` still declares the GitHub Packages feed from an earlier setup | Not needed: the `Orkeon` packages and the `orkeon` tool are on nuget.org, and the repo's `nuget.config` pins nuget.org as the only source. Remove the GitHub feed (`dotnet nuget remove source <name>`) — no token is required. |
