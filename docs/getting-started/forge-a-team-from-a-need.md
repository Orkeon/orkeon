...
- `forge.json` — the card's machine-readable twin: the session's slug, title, format, promotion instant and the brief — what `forge reopen` reads to rebuild a faithful session once the original is gone (nothing secret in it);
