...
Each reading says whose call it was. The orchestrator opens a scope for every task — the crew, the agent, the task — and the part of the run that makes a call names the work, which the reading carries as `operation`: `agent`, `manager`, `planning`, `rag`, `memory`, `flow`, `judge`, or the `ctx.llm.*` method. A RAG pipeline queried by an agent's tool stays that agent's. A call nothing claimed is counted all the same — `unattributed` in the engine's own reports, and no `operation` on the wire.
...
→ {"v":2,"seq":4,"ts":"…","kind":"cost.updated","crewId":"01K…","agentId":"writer","tokens":1840,"promptTokens":1600,"completionTokens":240,"model":"google/gemini-3.7-flash","provider":"openrouter","operation":"agent","cost":0.0021,"currency":"USD","costSource":"vendor"}
