...
**`v` moves only for a change an existing client would misread** — a field removed, renamed, or given another meaning. Adding is not such a change: a client ignores the kinds and fields it does not know (§6), so a new kind or a new payload field keeps `v: 2`. That is how `task.started` arrived, and how `cost.updated` gained the prompt/completion split, the vendor's cost and the estimated part (`estimatedTokens`).
...
| `run.finished` | `success`, `exitCode`, `tokens`, `durationMs`, `promptTokens`, `completionTokens`, `cacheHitTokens?`, `cacheMissTokens?`, `estimatedTokens?` | The run ends, with what it spent: the token total and its split — the sum of every `cost.updated` reading, so the two cannot disagree — the wall time, the cache pair when a provider measured it, and the estimated part when a provider left a call uncounted. |
...
Every generation call of the run, measured in one place: the provider factory wraps every provider it builds in a metering decorator, and a host that registers a provider of its own does it through `AddOrkeonLlmProvider`, which wraps it the same way. A call is therefore counted by the provider it went to, whoever made it — never by the caller, never twice. Before, only the agent loop and the scripting facade reported, and the manager, the planner, the RAG pipelines, the memory services, the flows, the judges and the agent's own retries spent tokens neither the live meter nor `run.finished` ever showed.

Each reading says whose call it was. The orchestrator opens a scope for every task — the crew, the agent, the task — and the part of the run that makes a call names the work: `agent`, `manager`, `planning`, `rag`, `memory`, `flow`, `judge`, or the `ctx.llm.*` method. A RAG pipeline queried by an agent's tool stays that agent's. A call nothing claimed is counted all the same, as `unattributed`.

Two things are not on the meter: **embedding calls**, which are not generation calls, and an `IChatClient` a host registers that is not built on an Orkeon provider. A text-only stream carries no usage at all: it is counted when it ends, as an estimate, under `estimatedTokens` — and an estimate is never priced. See [Known limitations](../reference/limitations.md).
