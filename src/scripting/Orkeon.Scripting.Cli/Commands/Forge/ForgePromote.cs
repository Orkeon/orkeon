...
internal static class ForgePromoter
{
    ...
    /// <summary>
    /// The launchers' header, after each shell's comment marker: the team they launch, under its
    /// <see cref="ArtifactName"/>. One spelling for the writer and for <c>forge rename</c>, which
    /// finds it again to rename the team in it (STUDIO-28).
    /// </summary>
    internal static string LauncherHeader(string teamName) =>
        $"Generated by Orkeon Forge for the team '{teamName}'.";

    /// <summary>
    /// A launcher's text with its header naming <paramref name="name"/> rather than
    /// <paramref name="formerName"/> (STUDIO-28): every other line — the sample inputs a user
    /// adapted included — as it was. Nothing else in a launcher names the team: its paths are
    /// anchored to the script's own folder.
    /// </summary>
    internal static string RenameLauncher(string launcher, string formerName, string name) =>
        launcher.Replace(LauncherHeader(formerName), LauncherHeader(name), StringComparison.Ordinal);
    ...
    private static void WritePosixLauncher(
        string destination, string teamName, ForgeSession session, ForgeBrief? brief,
        string? settingsReference, bool settingsIsRelative,
        IReadOnlyList<DeliverableMount> writeMounts)
    {
        var builder = new StringBuilder();
        builder.Append("#!/usr/bin/env sh\n");
        builder.Append(CultureInfo.InvariantCulture, $"# {LauncherHeader(teamName)}\n");
        ...
        IReadOnlyList<DeliverableMount> writeMounts)
    {
        ...
        builder.Append(CultureInfo.InvariantCulture, $"rem {LauncherHeader(teamName)}\r\n");
        builder.Append(ForgeSession.IsScriptFormat(session.Document.Format)
