...
internal static class ForgeTeamRenamer
{
    ...
    {
        var file = Path.Combine(directory, ConventionalNames.StudioTeamFile);
        if (!File.Exists(file))
            return;
        ...
        if (sidecar is null)
        {
            warnings.Add((ForgeErrorCodes.RenameSidecarUnreadable,
                $"{ConventionalNames.StudioTeamFile} could not be read: the name it records was left as it was."));
            return;
        }
