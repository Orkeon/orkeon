...
internal sealed class MountForm
{
    ...
    /// <summary>Creates an empty form over a directory probe (defaults to the real disk); the mount gets an id from the start (VFS-90).</summary>
    ...
    /// name it by. Generated for a new mount, carried over for an existing one, never edited.
    /// </summary>
    public MountId Id { get; private set; }

    /// <summary>Whether the id was generated by this form rather than read from the file.</summary>
    public bool IsIdGenerated { get; private set; }

    /// <summary>"Id: 01J… — generated" or "Id: 01J…" — the read-only line the dialog shows.</summary>
    public string IdLabel => IsIdGenerated
        ? string.Create(CultureInfo.InvariantCulture, $"Id: {Id} — generated")
        : string.Create(CultureInfo.InvariantCulture, $"Id: {Id}");
