...
public sealed class ForgeRenameTests : IDisposable
{
    ...
    public async Task Renaming_moves_both_folders_and_updates_every_title()
    {
        ...
        var sidecar = JsonNode.Parse(await File.ReadAllTextAsync(Path.Combine(Renamed, ConventionalNames.TeamSidecarFile), TestContext.Current.CancellationToken))!;
    ...
    public async Task A_team_without_a_session_is_renamed_too()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(imported, ConventionalNames.TeamSidecarFile), """{"name":"Import"}""", TestContext.Current.CancellationToken);
        var (_, host) = Machine(ForgePromotePlatform.Linux);
        ...
        Assert.Equal("Veille importée",
            JsonNode.Parse(await File.ReadAllTextAsync(Path.Combine(moved, ConventionalNames.TeamSidecarFile), TestContext.Current.CancellationToken))!["name"]!.GetValue<string>());
    ...
    public async Task A_taken_name_is_refused_and_says_what_holds_it()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(Renamed, ConventionalNames.TeamSidecarFile), """{"name":"Veille du matin"}""", TestContext.Current.CancellationToken);
        var (_, host) = Machine(ForgePromotePlatform.Linux);
        var before = Tree();
    ...
    private ForgeSession Adopt(string team, ForgePromotePlatform platform, string? schedule = "daily@07:30")
    {
        ...
        File.WriteAllText(Path.Combine(team, ConventionalNames.TeamSidecarFile),
            """
            {
              "name": "Ma veille",
