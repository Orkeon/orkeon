...
public sealed class MariaDbDatabaseToolTests : IDisposable
{
    private readonly MariaDbDatabaseTool _tool;

    public MariaDbDatabaseToolTests()
    {
        _tool = new MariaDbDatabaseTool(
    ...
    {
        Assert.Equal("mariadb_query", _tool.Name);
    }

    [Fact]
    public void ToolContract_HasCorrectDescription()
    {
        Assert.Contains("MariaDB", _tool.Description);
    }
    ...
    public async Task Validation_InjectsMySqlConnectorWhenProviderNameIsOmitted()
    {
        ...
        // With injection, it should fail at a later stage (DDL security policy).
        var request = new ToolCallRequest("mariadb_query", new Dictionary<string, object?>
        {
