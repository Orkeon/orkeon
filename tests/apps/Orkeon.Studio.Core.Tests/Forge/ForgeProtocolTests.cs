...
public class ForgeEventParserTests
{
    ...
    public void The_cli_golden_lines_parse_with_their_envelope()
    {
        ...
        {
            Assert.True(ForgeEventParser.TryParse(line, out var parsed), line);
            return parsed!;
        }).ToList();

        Assert.All(events, e => Assert.Equal(ForgeEventParser.KnownProtocolVersion, e.Version));
    ...
    {
        Assert.False(ForgeEventParser.TryParse(line, out var parsed));
        Assert.Null(parsed);
    }
    ...
    {
        Assert.True(ForgeEventParser.TryParse(
            """{"v":2,"seq":9,"ts":"2026-08-19T12:00:00Z","kind":"something.new","x":1}""", out var parsed));
        Assert.Equal("something.new", parsed!.Kind);
        Assert.Equal(2, parsed.Version);   // surfaced, not enforced — the client decides
    }
}
...
{
    private static ForgeEvent Event(string json)
    {
        Assert.True(ForgeEventParser.TryParse(json, out var parsed), json);
        return parsed!;
    }
    ...
    private static ForgeSessionModel FullCycle()
    {
        ...
        string[] stream =
        [
            """{"v":1,"seq":1,"ts":"t","kind":"session.started","slug":"veille","dir":"/ws/.orkeon/forge/veille","format":"yaml","resumed":false}""",
            """{"v":1,"seq":2,"ts":"t","kind":"stage.entered","stage":"brief","iteration":1}""",
            """{"v":1,"seq":3,"ts":"t","kind":"assistant.message","text":"Quel est le fournisseur ?"}""",
            """{"v":1,"seq":4,"ts":"t","kind":"brief.ready","brief":{"goal":"Résumer chaque matin les offres","acceptance":[{"id":"A1","statement":"Le résumé cite ses sources","kind":"must"},{"id":"A2","statement":"Moins d'une page","kind":"should"}]}}""",
            """{"v":1,"seq":5,"ts":"t","kind":"stage.entered","stage":"blueprint","iteration":1}""",
            """{"v":1,"seq":6,"ts":"t","kind":"blueprint.ready","blueprint":{"crew":{"name":"veille","goal":"Résumer"},"agents":[{"key":"collecteur","role":"Web Researcher","tools":["web_scrape"]},{"key":"redacteur","role":"Writer","tools":["file_write"]}],"tasks":[{"key":"collecte","description":"Collecter les offres","agent":"collecteur"},{"key":"resume","description":"Rédiger le résumé","agent":"redacteur"}],"rationale":"Deux rôles séparés."},"iteration":1}""",
            """{"v":1,"seq":7,"ts":"t","kind":"stage.entered","stage":"render","iteration":1}""",
            """{"v":1,"seq":8,"ts":"t","kind":"file.written","path":"crew/config.yaml","bytes":120}""",
            """{"v":1,"seq":9,"ts":"t","kind":"stage.entered","stage":"validate","iteration":1}""",
            """{"v":1,"seq":10,"ts":"t","kind":"validation.result","ok":true,"errors":[],"warnings":[],"attempt":0}""",
            """{"v":1,"seq":11,"ts":"t","kind":"stage.entered","stage":"test","iteration":1}""",
            """{"v":1,"seq":12,"ts":"t","kind":"run.started","run":1,"target":"crew"}""",
            """{"v":1,"seq":13,"ts":"t","kind":"task.completed","taskId":"collecte","agentRole":"Web Researcher","success":true,"durationMs":1200}""",
            """{"v":1,"seq":14,"ts":"t","kind":"run.finished","run":1,"success":true,"outputPath":"runs/1/output.md"}""",
            """{"v":1,"seq":15,"ts":"t","kind":"cost.updated","tokens":500,"budgetRemaining":null}""",
            """{"v":1,"seq":16,"ts":"t","kind":"stage.entered","stage":"diagnose","iteration":1}""",
            """{"v":1,"seq":17,"ts":"t","kind":"verdict.ready","score":0.4,"passing":false,"findings":[{"id":"F1","severity":"major","acceptance":"A1","statement":"Les sources ne sont pas citées"}],"suggestions":[{"target":"agent:redacteur","change":"citer chaque source","reason":"critère A1"}],"judge":"llm"}""",
            """{"v":1,"seq":18,"ts":"t","kind":"stage.entered","stage":"verdict","iteration":1}""",
            """{"v":1,"seq":19,"ts":"t","kind":"decision.needed","options":["accept","refine","abort"]}""",
        ];
    ...
    public void A_deterministic_verdict_says_judge_for_yourself_instead_of_a_fake_check()
    {
        var model = FullCycle();
        model.Feed(Event(
            """{"v":1,"seq":20,"ts":"t","kind":"verdict.ready","score":0.7,"passing":true,"findings":[{"id":"F-RUN","severity":"blocking","statement":"The crew did not complete its run."}],"suggestions":[],"judge":"deterministic"}"""));
    ...
    public void Refine_returns_to_propose_and_a_new_stage_clears_the_pending_decision()
    {
        ...
        model.Feed(Event("""{"v":1,"seq":20,"ts":"t","kind":"stage.entered","stage":"blueprint","iteration":2}"""));
    ...
    public void Promotion_and_errors_land_where_the_adopt_card_reads_them()
    {
        var model = new ForgeSessionModel();
        model.Feed(Event(
            """{"v":1,"seq":1,"ts":"t","kind":"promoted","path":"/solutions/veille","launcher":"run.sh","schedule":"schedule","install":"systemctl --user enable --now orkeon-veille.timer"}"""));
        model.Feed(Event(
            """{"v":1,"seq":2,"ts":"t","kind":"error","code":"FORGE-LLM-UNAVAILABLE","message":"no LLM","recoverable":true}"""));
        model.Feed(Event("""{"v":1,"seq":3,"ts":"t","kind":"session.finished","status":"ready","exitCode":0}"""));
