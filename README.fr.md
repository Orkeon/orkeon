...
- **Batteries incluses** — 75+ outils, 14 fournisseurs LLM, 6 stores de mémoire, vision, RAG, analyse de code : utilisables immédiatement, remplaçables via les ports de la Clean Architecture.
...
L'analyse qualité tourne sur un SonarQube local via `scripts/sonar-analyze.sh` — voir la [politique de quality gate](docs/fr/guides/quality-gate.md). Dernière passe (**5 septembre 2026** ; le rapport complet est un artefact local — `scripts/sonar-analyze.sh` l'écrit sous `sonarqube/`, que le dépôt ne versionne pas) : quality gate **OK**, **82,7 %** de couverture de lignes sur 162 k lignes (Application 90,7 %, Domain 88,3 %, Infrastructure 84,3 %), **0 bug, 0 vulnérabilité, 0 hotspot de sécurité**, dette technique **0 min**, 1,6 % de duplication. La même passe relève **406 code smells — tous de sévérité INFO** : 404 sont une seule suggestion d'analyseur xUnit répétée dans les projets de test (`xUnit2033`, « utilisez la valeur retournée par `Assert.Single` »), et les deux restants étaient des suggestions d'analyseur dans du code produit (`SYSLIB1054` dans `Orkeon.Studio.Core`, et un `SYSLIB1006` dans `Orkeon.Hosting` depuis corrigé). Aucun n'est une issue MINOR ou pire.
